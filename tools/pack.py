...
#
# The shim shadows the game's own `require`, so anything that is not a module
# name (a ModuleScript such as ReplicatedStorage.Modules.CombatUtil) must be
# handed back to the native one. Without that pass-through every game-module
# require raised "module not found" -- and since those calls sit inside
# pcall, the failure was silent and the hub ran blind.
PREAMBLE = """local __modules = {}
local __loaded = {}
local __native_require = require
local function require(name)
    if type(name) ~= "string" then return __native_require(name) end
    local cached = __loaded[name]
    if cached ~= nil then return cached end
    local factory = __modules[name]
    if not factory then
        error("[Strawberry Hub] module not found: " .. tostring(name), 2)
    end
...
def bundle(modules: list[tuple[str, pathlib.Path]], entry: str = ENTRY) -> str:
    out = ["-- Strawberry Hub -- bundle generated by tools/pack.py. Do not edit.", PREAMBLE]
    for name, path in modules:
...
-- Packed distribution. Do not edit: edit src/ and re-pack.
